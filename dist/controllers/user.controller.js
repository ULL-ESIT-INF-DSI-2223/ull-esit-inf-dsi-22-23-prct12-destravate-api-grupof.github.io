...
export const addTrackToHistory = async (req, res) => {
    try {
        ...
        const user = await userModel.findByIdAndUpdate(userId, { $push: { historicTracks: { date: new Date(date), track: track } } }, { new: true, useFindAndModify: false } // new: true para devolver el documento actualizado, useFindAndModify: false para utilizar el método findOneAndUpdate de Mongoose en lugar del método findAndModify de MongoDB
        );
        if (!user) {
