...
export const addTrackToHistory = async (req, res) => {
    try {
        ...
        const group = await groupModel.findByIdAndUpdate(groupId, { $push: { historicTracks: { date: new Date(date), track: track } } }, { new: true, useFindAndModify: false } // new: true para devolver el documento actualizado, useFindAndModify: false para utilizar el método findOneAndUpdate de Mongoose en lugar del método findAndModify de MongoDB
        );
        if (!group) {
...
export const addUserToGroup = async (req, res) => {
    try {
        ...
        // Encuentra el usuario y actualiza su array historicTracks
        const group = await groupModel.findByIdAndUpdate(groupId, { $push: { participants: { date: new Date(date), user: user } } }, { new: true, useFindAndModify: false } // new: true para devolver el documento actualizado, useFindAndModify: false para utilizar el método findOneAndUpdate de Mongoose en lugar del método findAndModify de MongoDB
        );
        if (!group) {
            return res.status(404).send('group not found');
        }
